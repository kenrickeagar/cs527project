...
<a href="HomePage.jsp">Go Back Home </a>
	...
	<br>
		<form method="get" action="showShirtList.jsp">
			<table align = 'center'>
				<tr>
					<td>Sort By</td>
	...
	<br>
		<form method="post" action="showShirtList.jsp">
			<table align = "center">
				<tr>    
					<td>Enter Keyword</td><td><input type="text"  name="search_shirts"></td>
				</tr>
			</table>
...
<%
			...
			String search_shirts = request.getParameter("search_shirts");
			
			String str;
			ResultSet result = null;
		if(search_shirts != null ){
	
	str = "SELECT * FROM Items WHERE cid = 'SHIRTS' AND item_name LIKE CONCAT('%',?,'%')";
	PreparedStatement pstmt = con.prepareStatement(str);
    pstmt.setString(1, search_shirts);
	 result = pstmt.executeQuery();
			...
			if(sort_by == null || sort_by.isBlank() || sort_by.isEmpty() || sort_by.equals("No Selection")){
				...
				str = "SELECT * FROM Items WHERE cid = 'SHIRTS' ORDER BY unit_price ASC ";
				PreparedStatement pstmt = con.prepareStatement(str);
				 result = pstmt.executeQuery();
			} else if(sort_by.equals("unit_price DESC")){
				str = "SELECT * FROM Items WHERE cid = 'SHIRTS' ORDER BY unit_price DESC ";
				PreparedStatement pstmt = con.prepareStatement(str);
				 result = pstmt.executeQuery();
			}else if(sort_by.equals("item_name")){
				str = "SELECT * FROM Items WHERE cid = 'SHIRTS' ORDER BY item_name ";
				PreparedStatement pstmt = con.prepareStatement(str);
				 result = pstmt.executeQuery();
			}else if(sort_by.equals("subcatAttribute")){
				str = "SELECT * FROM Items WHERE cid = 'SHIRTS' ORDER BY subcatAttribute ";
				PreparedStatement pstmt = con.prepareStatement(str);
				 result = pstmt.executeQuery();
			}
			}
